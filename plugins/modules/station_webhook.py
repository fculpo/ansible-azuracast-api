...
short_description: Manage one AzuraCast station webhook
version_added: "0.2.0"
description:
  - Manages a single webhook for one AzuraCast station.
options:
  base_url:
    description:
  ...
  api_key:
    description:
      - AzuraCast API key with permission to manage station webhooks.
    required: true
    type: str
  station_short_name:
    description:
  ...
  openapi_contract:
    description:
      - Optional AzuraCast OpenAPI document used to validate webhook endpoint capabilities.
    type: raw
  state:
    description:
      - Whether the webhook should exist.
    type: str
    choices:
      - present
      - absent
...
def station_webhook_spec():
    return station_resource_spec(
        "station webhook",
        "/api/station/{station_id}/webhooks",
        "/api/station/{station_id}/webhook/{id}",
        "name",
    )


def build_desired_station_webhook(name, resource, sensitive_resource=None):
...
def main():
    ...
    try:
        desired = build_desired_station_webhook(
            module.params["name"],
