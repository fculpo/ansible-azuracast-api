...
short_description: Manage one AzuraCast station remote relay
version_added: "0.2.0"
description:
  - Manages a single remote relay for one AzuraCast station.
options:
  base_url:
    description:
  ...
  api_key:
    description:
      - AzuraCast API key with permission to manage station remotes.
    required: true
    type: str
  station_short_name:
    description:
  ...
  display_name:
    description:
      - Stable remote display name to manage.
    required: true
    type: str
  resource:
    description:
      - Desired remote payload.
    type: dict
    default: {}
  sensitive_resource:
    description:
  ...
  openapi_contract:
    description:
      - Optional AzuraCast OpenAPI document used to validate remote endpoint capabilities.
    type: raw
  state:
    description:
      - Whether the remote should exist.
    type: str
    choices:
      - present
      - absent
...
EXAMPLES = r'''
- name: Ensure remote relay exists
...
def station_remote_spec():
    return station_resource_spec(
        "station remote",
        "/api/station/{station_id}/remotes",
        "/api/station/{station_id}/remote/{id}",
        "display_name",
    )


def build_desired_station_remote(display_name, resource, sensitive_resource=None):
...
def main():
    ...
    try:
        desired = build_desired_station_remote(
